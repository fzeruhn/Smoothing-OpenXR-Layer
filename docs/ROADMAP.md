...
2. Add **hardware queue isolation (EAC-safe, OpenXR-level only)** — *next active task*:
   ...
   - **Step A [Experimental] — probe (zero cost, try first):** in `xrCreateSession`, call `vkGetDeviceQueue(device, queueFamilyIndex, queueIndex+1, &rtQueue)`; if non-null and ≠ app queue, assign to RT and skip Steps B/C. Works when the app requested multiple queues (common in AAA engines).
