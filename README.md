...
A custom [OpenXR API layer](https://www.khronos.org/registry/OpenXR/specs/1.0/html/xrspec.html#api-layers) that implements high-performance motion smoothing for VR. It sits between the OpenXR runtime and the application, intercepting `xrEndFrame` to synthesize intermediate frames before they reach the compositor.
...
- **Visual Studio 2026** with C++ desktop workload + NuGet package manager
...
After the first build, copy the CUDA runtime DLL to the output directory:
```
...
Three headless test executables validate the completed pipeline subsystems:
...
`ofa-test.exe` also writes `ofa-test-output.png` (color-coded flow field) to the current directory.
